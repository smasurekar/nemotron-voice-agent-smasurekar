...
| Key | Effect |
|---|---|
| `agent.mode` | `frontend_backend` (default) or `backend_only` (standalone, stateful backend) |
...
90 offline tests, no network, no GPU, no credentials: every LLM is a scripted `FakeChatClient`.
